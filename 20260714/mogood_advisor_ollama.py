...
標準ライブラリのみ使用（pip install 不要。ただしデータ読込に openpyxl が必要）。
"""
...
MODEL = "gemma3:1b"  # 例: "qwen3:8b", "qwen3:4b", "gemma3:4b", "llama3.1:8b" などに変更可
...
def build_user_prompt(dish, preference_text):
    # レシピに関係するカテゴリだけに絞ってLLMに渡す
    # （ローカルLLMはコンテキスト長が短いため、全783商品は渡さない）
...
def advise_ollama(dish, preference_text):
    payload = {
        ...
        "options": {"temperature": 0.2, "num_ctx": 16384},  # 長い商品リストに対応
        "messages": [
            {"role": "system", "content": SYSTEM_PROMPT},
            {"role": "user", "content": build_user_prompt(dish, preference_text)},
        ],
    }
    # think は thinking 対応モデル（qwen3 等）のみ有効。gemma3 等に送ると400になるため分岐する
    if MODEL.startswith("qwen3"):
        payload["think"] = False  # qwen3の「思考」を省略して大幅高速化
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*62}\n● 料理: {dish} ／ 好み: {pref}\n{'='*62}")
    advise_ollama(dish, pref)  # 生成文はストリーミングで逐次表示される
